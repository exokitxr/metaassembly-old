...
HANDLE startChrome(const std::string &indexHtmlPath) {
  std::string cmd(R"EOF(Chrome-bin\chrome.exe --enable-features="WebXR,OpenVR" --disable-xr-device-consent-prompt-for-testing --load-extension="..\..\extension" --whitelisted-extension-id="glmgcjligejadkfhgebnplablaggjbmm"" --user-data-dir="Chrome-bin\userdata" --app=)EOF");
  cmd += indexHtmlPath;
